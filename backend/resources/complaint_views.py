...
class SetSatisfaction(Resource):
    ...
    def post(self, complaint_id):
        ...
        current_user_id = get_jwt_identity()
        complaint = Complaint.query.filter_by(id=complaint_id, user_id=current_user_id).first()
...
@complaint_ns.route('/admin/allcomplaints')
class AdminAllComplaints(Resource):
    @jwt_required()
    def get(self):
        """Allow admins to see all complaints from all departments."""
        current_user_id = get_jwt_identity()
        user = User.query.get(current_user_id)

        if not user or not user.is_admin:
            return {'message': 'Unauthorized: Only admins can access this'}, 403
